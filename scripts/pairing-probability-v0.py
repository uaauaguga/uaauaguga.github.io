...
def RNAstructure(sequence,prefix,shape=None):
    ...
    if shape is None:
        command = ["/BioII/lulab_b/jinyunfan/anaconda3/envs/bioinfo_py27/bin/partition","-","--maxdistance","200",partition_file_path]
    else:
        shape_path = prepareSHAPE(shape,prefix)
        command = ["/BioII/lulab_b/jinyunfan/anaconda3/envs/bioinfo_py27/bin/partition","-","--maxdistance","200","--SHAPE",shape_path,partition_file_path]
        print(" ".join(command))
